...
func TestCreateCommit_EmptyNotAllowed(t *testing.T) {
	repo, _ := createTestRepoWithCommit(t)

	// Try to create commit without changes and without AllowEmpty
	// Note: go-git's AllowEmptyCommits=false does NOT prevent empty commits,
	// it just means they're not explicitly allowed. Empty commits still succeed.
	hash, err := repo.CreateCommit(CommitOptions{
		Author:  "Test Author",
		Email:   "test@example.com",
		Message: "Empty commit without explicit AllowEmpty",
	})
	// With go-git, this succeeds (empty commits are allowed by default)
	require.NoError(t, err)
	assert.NotEmpty(t, hash)
}
